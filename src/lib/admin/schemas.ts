...
export const projectSchema = z.object({
  ...
  projectSize: optionalText(100), deliveryMethod: optionalText(120), services: z.string().max(3000), heroImageUrl: optionalText(2000), heroImageAlt: optionalText(300),
  gallery: z.string().max(12000), metrics: z.string().max(5000), seoTitle: optionalText(70), seoDescription: optionalText(180), status: z.enum(["draft","published","archived"]),
...
export const serviceSchema = z.object({
  ...
  description: z.string().trim().max(10000), capabilities: z.string().max(5000), relatedProjects: z.string().max(3000), imageUrl: optionalText(2000), imageAlt: optionalText(300),
  seoTitle: optionalText(70), seoDescription: optionalText(180), active: z.boolean(), featured: z.boolean(), sortOrder: z.coerce.number().int().min(-1000).max(1000),
});
